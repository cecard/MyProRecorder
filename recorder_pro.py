...
class ProfessionalRecorder:
    def __init__(self, root):
        ...
        if not os.path.exists(self.ffmpeg_path):
            messagebox.showerror("Error", f"FFmpeg missing at:\n{self.ffmpeg_path}")
    ...
    def _start_recording_thread(self):
        ...
        try:
            ...
            self.root.after(0, lambda: self._try_ffmpeg_engine(p, stream, audio_args))
    ...
    def _ask_user_audio(self, p, stream, error_msg):
        ...
        if not ans:
            ...
            self._reset_ui()
            return
        self._try_ffmpeg_engine(p, None, [])

    def _try_ffmpeg_engine(self, p, stream, audio_args):
        """ 尝试启动录制，优先使用 ddagrab，失败则降级 gdigrab """
        
        # 保存到桌面！
        desktop_path = get_desktop_path()
        filename = os.path.join(desktop_path, f"Recording_{int(time.time())}.mkv")
        ...
        common_args = ['-framerate', '30']
        if mouse_flag == '0': common_args.extend(['-draw_mouse', '0']) # ddagrab 特有
        
        # 选区处理
        if self.region:
            x, y, w, h = self.region
            x, y, w, h = x//2*2, y//2*2, w//2*2, h//2*2
            common_args.extend(['-offset_x', str(x), '-offset_y', str(y), '-video_size', f"{w}x{h}"])
        
        # 尝试 1: ddagrab (Windows 10/11 高性能)
        success = self._run_ffmpeg_process(filename, audio_args, ['-f', 'ddagrab'] + common_args + ['-i', 'desktop'], stream)
        
        if not success:
            print("ddagrab failed, trying gdigrab...")
            # 尝试 2: gdigrab (传统兼容模式)
            # gdigrab 的鼠标参数是 draw_mouse 1/0，且要在 input 之前
            gdi_args = ['-f', 'gdigrab', '-framerate', '30', '-draw_mouse', mouse_flag]
            if self.region:
                # gdigrab 的选区参数略有不同，通常是一样的，但重新构建以防万一
                x, y, w, h = self.region
                x, y, w, h = x//2*2, y//2*2, w//2*2, h//2*2
                gdi_args.extend(['-offset_x', str(x), '-offset_y', str(y), '-video_size', f"{w}x{h}"])
            gdi_args.extend(['-i', 'desktop'])
            
            success = self._run_ffmpeg_process(filename, audio_args, gdi_args, stream)
            
            if not success:
                messagebox.showerror("Error", "Failed to start recording with both ddagrab and gdigrab.")
                self.stop_recording()

    def _run_ffmpeg_process(self, filename, audio_args, video_input_args, stream):
        cmd = [self.ffmpeg_path, '-y'] + audio_args + video_input_args + \
    ...
    def _finish_stop(self):
        ...
        messagebox.showinfo("Done", f"Video Saved to Desktop!\n{desktop}")
